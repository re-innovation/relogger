  ...
  21/6/17 Vext Works OK - Calibration required
  21/6/17 VBatt Works OK
  21/6/17 Hall Effect I Works OK- Calibrationr required
  23/6/17 Opto Sensor Works OK
  23/6/17 Hall effect Anemometer Works OK - Need debouce (delay?)
  23/6/17 Anemometer Works OK
  23/6/17 Added - No current sensor attached (value out of range)  Must use corrent out-of-range value (50A in this case)
      
  TO DO

  Undervoltage protection error
  SD error display
...
#define current1Pin SINGLE_2  // Current from a hall effect sensor ADC 2
...
boolean underVoltageFlag = LOW; // For undervoltage check LED
boolean sdErrorFlag = LOW; // For SD error check LED
//**********STRINGS TO USE****************************
String comma = ",";
...
void pulse1(void)
{
  ...
  // If the anemometer has spun around break out of sleep
  pulse1Flag=HIGH;  
...
void setup()
{
  ...
  PCintPort::attachInterrupt(ACFREQ, &pulse2, RISING); 
  attachInterrupt(RTCinterrupt, RTC, RISING);
  
  dataCounter = 0;  // Reset the data counterbefore starting to get first data OK.
}
...
void loop()
{
  ...
  if((pulse1Flag==HIGH||pulse2Flag==HIGH)&&calibrateFlag==HIGH)
  {
    if(pulse1Flag==HIGH)
    {
      PCintPort::detachInterrupt(ANEMOMETER1);  // add more attachInterrupt code as required
      int anamomCount1 = 0;
      
      for(int j=0;j<4;j++)
      {
        anamomCount1+=digitalRead(ANEMOMETER1);
      }       
      if(anamomCount1>2)
      {
...
//        Serial.flush(); 
    ...
    if(pulse2Flag==HIGH)
    {
      PCintPort::detachInterrupt(ACFREQ);  // add more attachInterrupt code as required
      int anamomCount2 = 0;
      
      for(int j=0;j<4;j++)
      {
        anamomCount2+=digitalRead(ACFREQ);
      }       
      if(anamomCount2>2)
      {
        // Increment the pulse counter
...
//        Serial.flush(); 
      ...
      PCintPort::attachInterrupt(ACFREQ, &pulse2, RISING);  // add more attachInterrupt code as required
...
//      dtostrf(externalVoltage,2,2,ExternalVoltStr);     // Hold the battery voltage as a string             
      externalVoltage=adc.read(MCP3208::voltagePin);     
      externalVoltage=(float(externalVoltage)-3.8f)/51.98285f;     
...
//      current1 = (current1*200.0f)/1.25f;
  ...
  //    current1 = (current1 * 50.0f) / 0.625f;    
      ...
      current1 = (current1*40.0f*(float(ADC_VREF)/float(batteryVoltage)))/1000000.0f;
...
//      Serial.println(current1);      
      ...
      dataString += comma;
      // Here we decide if the current sensor is attached or not
      // If reading is > or < 50A then ignore - N/C
      // 50A at 40mV/A = 2000mV
      if(current1>50||current1<-50)
      {
       dataString += "N/C"; 
      }
      else
